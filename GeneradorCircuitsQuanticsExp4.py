...
class GeneradorCircuitsQuanticsExp4():
    ...
    def generate_circuit(self, data):
        ...
        v_depth_losses.append(self.normalized_depht_loss)
        v_gates_losses.append(self.normalized_gates_loss)
            ...
            for step in range(max_steps):
                ...
                for _ in range(iterations_per_step):
                    ...
                    if accept_new_state:
                        ...
                        v_depth_losses.append(v_depth_losses[-1])
                        v_gates_losses.append(v_depth_losses[-1])

                    losses.append(loss)
                    if loss == 0:
                        break
                #Han acabat 100 iteracions, calculem nous parametres
                
                #actualitzem el nombre de gates to reorder
                v_gates_to_reorder.append(gates_to_reorder)
                ...
                #guardem les dades
                v_ratios.append(ratio)
                v_temperatures.append(temperature)
                ...
                #a = sum( (x_n - m)* (x_n - m) for x_n in losses)
                #b = n*n - n
                #coef1 = math.sqrt(a/b)
                #coef2 = val * m

                #v_coefs.append((coef1, m, step*iterations_per_step))
                ...
                eps = 1e-13
                steps_back_to_check = 100#15
                if step > steps_back_to_check and all([abs(loss_each_step[-i] - loss_each_step[-i-1]) < eps for i in range(1,steps_back_to_check)]):
                    end_reason = f"all([abs(loss_each_step[-i] - loss_each_step[-i-1]) < {eps} for i in range(1,10)])"
                    break
        ...
        #    "losses": losses, 
        #    "ratios": v_ratios, 
        #    "temperature": v_temperatures,
        #    "depht_losses": v_depth_losses,
        #    "gates_losses": v_gates_losses,
        #    "gates_to_reorder": v_gates_to_reorder,
        #    "coefs": v_coefs,   # coef1, coef2/v iteration (step*numits)
        #    "final_iteration": iteration
        #    }